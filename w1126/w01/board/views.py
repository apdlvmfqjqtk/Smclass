...
def breply(request, bno):
  if request.method == "GET":
    ...
    context = {'rmsg':bno}
    return redirect('/board/blist/', context)
...
def bupdate(request, bno):
  if request.method == "GET":
    ...
    context = {'umsg':bno}
    return redirect(f'/board/bview/{bno}/', context)
...
def bview(request, bno):
  ...
  ## 이전글, 다음글
  # prev_qs = Board.objects.filter(Q(bgroup__lt=qs.bgroup,  bstep__lte=qs.bstep) | Q(bgroup=qs.bgroup,bstep__gt=qs.bstep_)).order_by("-bgroup", "bstep").first()
  ...
  if request.COOKIES.get("cookie_boardNo") is not None:
    cookies = request.COOKIES.get("cookie_boardNo")  # 1|5|4|2
