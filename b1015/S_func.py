...
def title_program():
  ...
  print("4. 학생성적검색")
  print("5. 학생성적삭제")
  print("6. 등수처리")      # 보통 DB영역
  print("7. 학생성적정렬")  # 보통 DB영역
  print("0. 프로그램 종료")
  print("-"*60)
...
def stu_select(students):
  while True:
    flag = 0
    name = input("검색하려는 이름을 입력하세요.(0.이전 화면 이동) : ")
    if name == "0":
      break
    ...
    if flag == 0:
      ...
      print(f"{name} 이름으로 {len(sArr)}명이 되었습니다")
      stu_output(sArr)
#-------------------------------
...
def stu_delete(students):
  while True:
    print("[ 학생 성적 삭제 ]")
    name = input("찾으려는 학생 이름을 입력하세요. (0.이전 메뉴): ")
    if name == "0":
      break
    flag = 0
    sArr = []
    for idx,s in enumerate(students):
      if s['name'] == name:
        flag = 1
        print(f"{name} 학생 성적을 삭제하시겠습니까?(삭제 시 복구 불가능)")
        print("1.삭제 2.취소 : ")
...
def stu_sort(students):
  while True:
    ...
    if choice == "1":
      ...
      students.sort(key=lambda x:x['name']) # 리스트
    elif choice == "2":
      ...
      students.sort(kry=lambda x:x['no'])
    elif choice == "0":
      print("이전페이지로 이동합니다.")
      break
    print("정렬이 완료되었습니다.")
