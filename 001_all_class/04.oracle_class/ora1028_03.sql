...
-- number(3)
--varchar2(20) -- 문자열
--no, name, kor, eng, math, total, avg, rank 칼럼을 구성해서 테이블을 생성   
...
select employee_id,emp_name,salsry from employees;
...
select * from employees;
-- 사원번호, 사원명, 전화번호, 입사일(create_date) 컬럼만 출력.
-- 사원번호, 사원명, 직급번호(job id)가 SH_CLERK만 출력
SELECT employee_id, emp_name, phone_number, create_date FROM employees;
