...
sss = ss.replace(" ","") # replace는 원본을 변경할수는 없고 변수에 담아서 출력해야 한다
print(sss)
# ss.split() 분리 // ss.join() 합치기 
...
# # 리스트 내포
# # [ 값 for문 조건식 ]순서로 올 수 있다(한 줄 for문에만)
...
# # 함수를 이용한 모듈화 (유지보수가 쉽다, 역할분담이 쉽다, 각자 만든 작업물을 합치기도 쉽다.)
# if choice == "1":
#   stu_input()
# elif choice == "2":
#   stu_output()
